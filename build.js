...
if ( typeof config.resumePdf === 'object' && typeof config.resumePdf.dest === 'string' ){
  config.redirects[ '/resume' ] = config.resumePdf.dest
}
...
const siteBuild = Metalsmith(__dirname)
  ...
  .use(msIf(
    //( typeof config.resumePdf === 'object' && typeof config.resumePdf.dest === 'string' ),
    true,
    redirect({
      '/resume': config.resumePdf.dest,
      '/github': 'https://github.com/jhyland87'
    })
  ))
  .build(function (err) {
